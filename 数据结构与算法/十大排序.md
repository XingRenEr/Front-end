...
**改进**：如果 arr[middle] 小于等于 arr[middle+1] ，我们就认为数组已经是有序的并跳过 merge() 方法。任意有序的子数组算法的运行时间就变为线性的了，即最佳时间复杂度为 O(n)
...
function mergeSort(arr) {
    ...
    var middle = Math.floor(len / 2),
        left = arr.slice(0, middle),
        right = arr.slice(middle);
    return arr[middle] <= arr[middle + 1] ? [...mergeSort(left), ...mergeSort(right)] : merge(mergeSort(left), mergeSort(right));
}
```
...
> 应用：[数组中的第K个最大元素](https://leetcode-cn.com/problems/kth-largest-element-in-an-array/)
