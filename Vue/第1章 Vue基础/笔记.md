...
| &emsp;&emsp;[4.5 v-show](#four-five) |
| [五 列表渲染](#five) |
...
防止XSS，csrf：(1)前端过滤(2)后台转义(<> &lt &gt)(3)给cookie加上属性http  
...
```
1. 修改class：控制台```vm.classobj.a = false```  
2. 对象写法的问题：```vm.classobj.c = true```无法添加新的class c。  
原因：涉及到Vue的更新原理。classobj有get a,set a,get b,set b方法，因此只能对状态中已经定义的属性进行true/false的设置。  
![](笔记_files/10.jpg)  
...
比class麻烦  
v-bind绑定(:style为v-bind:style的简写)后，后面是JS的地盘  
```HTML
<body>
	<div id="box">
		<button @click="handleClick()">click</button>
...
```
对象语法：想修改之前，可以先看一看原来的值是什么格式，再修改。不能新加样式(原理同class绑定)。  
![](笔记_files/11.jpg)  
数组语法：样式的增加和移除举例如下  
![](笔记_files/12.jpg)  
...
![](笔记_files/18.jpg)  
![](笔记_files/19.jpg)  
## <a id='four-four'></a>4.4 template v-if ,包装元素template 不会被创建
> [返回目录](#zero)

## <a id='four-five'></a>4.5 v-show
> [返回目录](#zero)
...
![](笔记_files/23.jpg)  
在列表中间插入新的节点55555  
Vue原理：数据拦截之后，创建新的虚拟DOM节点对比老的节点，对比的时候(通过索引值来对比不合理，需要修改一次节点，再在后面创建一个节点)key值登场——  
...
![](笔记_files/33.jpg)  
该方法还可用于使用对象进行class绑定时，添加新class(之前讲过不能使用```vm.classobj.c = true```添加新的class c)  
![](笔记_files/34.jpg)  
- ```splice```
![](笔记_files/31.jpg)  
![](笔记_files/32.jpg)  
...
```
在模板中没有```this```：在模板中访问状态，从来不加this，因为它会自动找到当前实例来进行初始化。  
当代码很长，使用该方法使得模板的可维护性极差，很多JS代码混入到HTML代码中  
### 写函数名 
handleClick  
【函数名、函数表达式】  
不传入参数的前提下，两种方法相同  
...
不做任何处理时，父元素的方法为何会触发——父元素监听到从子元素冒上来的事件  
不是阻止冒泡  
4. ```once```事件只执行一次  
应用：抽奖只有一次机会  
...
并不是所有按键都和名字对应好的。也可使用键值，如enter和13等价  
...
```HTML
<p>你喜欢的运动？
	<input type="checkbox" v-model="checkgroup"/>游泳
	<input type="checkbox" v-model="checkgroup"/>滑冰
	<input type="checkbox" v-model="checkgroup"/>长跑
</p>
```
【问题】  
一选全选，绑定的数组checkgroup啥都没有  
![](笔记_files/47.jpg)  
【解决】  
要给多选框设置value值  
```HTML
...
## <a id='seven-two'></a>7.2 (017~020)-购物车  
> [返回目录](#zero)

【bind:value】
![](笔记_files/48.jpg)  
绑定内容可以是data.id  
可以是data.number*data.price，但是之后data数据可能还有其他用途，所以不这样绑定  
这里绑定data  
```HTML
<ul>
	<li v-for="data in datalist">
...
```
3. ```trim```  
去除表单中的首尾空格(注册用户名的场景可以使用)，原始方法是使用正则表达式  
```HTML
...
## <a id='nine-two'></a>9.2 计算缓存 VS methods
> [返回目录](#zero)
...
```
计算属性定义起来像一个函数一样，使用起来像一个属性一样(不加括号)。依赖的状态改变了，计算属性会重新计算一遍  
...
计算属性与普通方法唯一的区别就是调用时是否使用小括号？  
大错特错  
【计算属性与普通方法的区别】  
计算属性的性能比普通方法高一些。  
在一个页面中，若一个计算结果要多次使用，计算属性性能的优越之处就体现出来了  
普通方法只要调用一次，方法就会重新执行一次；而计算属性计算过以后，会把计算结果缓存下来(只是在内存中的一个缓存，刷新页面就丢了)，当在页面多次使用时，可以保证只计算一次  
...
Vue的更新原理：  
数据修改后被ES5中固带的方法Object.defineProperty的get set拦截到后，通知watcher，watcher去重新渲染函数。  
渲染函数需要做很多事情：创新新的虚拟dom，对比老的虚拟dom节点(该过程称为diff)，做出一个补丁，打到真实的DOM上，真实DOM节点就被更新了。  
![](笔记_files/60.jpg)    
...
旧的和新的虚拟DOM节点分别为：  
![](笔记_files/64.jpg)  
先对比tag：p不同于div——虚拟DOM节点发生了翻天覆地的变化，不可能复用了，于是不再进行text的对比，直接创建新的节点。  
若标签相同，则要继续对比内容，再决定复用/创建新节点：  
![](笔记_files/65.jpg)  
![](笔记_files/66.jpg)  
...
### (033)-子传父应用
【实现一个功能】  
有一个按钮，一个装着列表的组件。点击按钮，隐藏列表，再次点击，显示。  
```HTML
<div id="box">
...
![](笔记_files/78.jpg)  
mytext就是自定义的属性值  
鼠标放到input上，发现输入框被选中，即mytext对应着输入框原生DOM节点  
![](笔记_files/79.jpg)  
![](笔记_files/80.jpg)  
.value即可取到该输入框节点的value值  
```js
...
<body>
	...
	<script type="text/javascript">
		...
		Vue.component("weixinauthor", {
			...
			methods: {
				handleClick() {
					bus.$emit("weixinmessage", this.mytext)
				}
			}
		})
...
Vue提供了一个叫做component的动态组件，帮助方便地管理多个组件的显示与否  
若没有动态组件的帮助，需要使用多个状态，通过设置true/false来动态地显示/隐藏  
```HTML
<body>
	<div id="box">
...
![](笔记_files/93.jpg)  
【作用】  
混合父组件的内容与子组件自己的模板(将父组件的内容混合到子组件自己的模板中)-->内容分发  
【为什么要使用插槽】  
栗子：封装一个轮播组件，列表中的内容很可能是动态获取的，不能给写死。这时就需要使用插槽  
```html
<body>
	<div id="box">
		<swiper>
...
而通过插槽，可以将属于父组件的button按钮插入到子组件中，该按钮还可以访问到父组件的状态isShow  
```html
<body>
	<div id="box">
		<navbar>
...
data.img | kerwinpath 表示将数据data.img送到过滤器kerwinpath中去处理，返回处理完成的结果。|为管道符  
```HTML
...
```
为了防止和其他人有冲突，不使用类选择器选择swiper-container类，而是添加一个a类，选择a类  
...
3. 放在updated()中也不是最优方法，因为各种操作都可能触发updated，这样swiper就会被触发多次。最好自定义一个Swiper组件  
...
## <a id='seventeen-two'></a>17.2 (050)-指令轮播  
> [返回目录](#zero)

之前使用swiper时的问题：swiper实例化过早。  
解决方法：  
1. 使用根节点updated生命周期初始化swiper；  
2. 使用组件，为swiper组件动态绑定key值。  
3. 因为自定义指令能够准确的知道使用该指令的节点什么时候才实例化完，插入到节点中。因此非常适合用自定义指令的来解决该问题：  

【方法一：vue swiper轮播】  
使用根节点updated生命周期初始化swiper  
```html
<body>
	<div id="box">
		<div class="swiper-container a">
...
```
相应的，为指令传值应该传入一个对象(因为想传两个参数——index和数组的长度)  
```html
<div id="box">
	<div class="swiper-container a">
		<div class="swiper-wrapper">
...
[单组件文件](https://cn.vuejs.org/v2/guide/single-file-components.html)  
文件扩展名为 .vue 的 single-file components (**单文件组件**)  
![](笔记_files/104.jpg)  

将.vue转换成浏览器认识的HTML CSS JS的预处理器(需要配合相关插件)：**Babel**，预处理器还可以将浏览器不认识的ES6转成ES5...  
其实，最终的转接的由**webpack**完成的。只需要将webpack引入进来，写好webpack一系列的配置文件，就可以将Vue文件直接转换成HTML文件，ES6代码直接转换成ES5代码。  
**Vue-cli脚手架**把webpack的配置文件写好了(几百行代码)，因此引入Vue-cli即可进行单文件组件开发  
...
![](笔记_files/112.jpg)  
然后就开始使用npm安装...安装到node-sass卡住的话，ctrl+C，删除node_modules文件夹，使用cnpm -i来安装package.json配置文件中的包即可  
...
![](笔记_files/122.jpg)  
【原理】  
为元素添加了独一无二的永远不会和别人冲突的属性，样式则加上了属性选择器，实现了局部CSS作用域的效果  
![](笔记_files/123.jpg)  
...
打包成一个dist文件夹  
![](笔记_files/124.jpg)  
如果会Node开发，将dist文件夹放到public文件夹下  
![](笔记_files/125.jpg)  
【演示Node开发中的打包】  
...
为啥需要配置？  
目标：做出一个网站  
要求：需要跟后端接口对接，而且后端接口一定会有跨域的问题。  
以[猫眼电影](https://m.maoyan.com/#movie)接口为例  
![](笔记_files/131.jpg)  
[猫眼电影接口地址](https://m.maoyan.com/ajax/movieOnInfoList?token=&optimus_uuid=DFDCE310105211EB85FB01669405DCB692D63CDB1B7E46658BFD0EA8BD4303A1&optimus_risk_level=71&optimus_code=10)  
接口返回的是JSON数据  
...
跨域被阻止了  
![](笔记_files/132.jpg)  
由于没有JSONP的接口，可以试一下使用![](笔记_files/133.jpg)接口  
后端设置成这样就可以随便请求数据了  
![](笔记_files/134.jpg)  
然鹅猫眼电影没有以上两个接口  
【解决方法——配置反向代理】  
