...
- 全局函数无法查看局部函数的内部细节，但局部函数可以访问其外部的函数作用域中的变量，直至全局函数作用域中的变量
...
| - | - | - | - | - | - | - | - |
|[Cookie](#1.29)					|可设置失效时间，否则默认为关闭浏览器后失效						|4k				|在所有同源窗口共享														|每次都会携带在http 头中，如果使用cookie 保存过多数据会带来性能问题	|需要程序员自己封装，原生的cookie 接口不友好|会话状态管理（如用户登录状态、购物车、游戏分数或其它需要记录的信息）；个性化设置（如用户自定义设置、主题等）；浏览器行为跟踪（如跟踪分析用户行为等）——MDN	|可以限制cookie只属于某个路径下	|
...
安全性：LocalStorage和SessionStorage在容易受到**XSS攻击**，不建议把敏感的个人信息（包括SessionID）存储在Web Storage中。
...
- 应用层协议
  ...
  - [HTTPS](#4.2)
    - 加密过程
- [DNS](#4.14)
- 传输层协议——[TCP 与 UDP](https://zhuanlan.zhihu.com/p/108822858)
  - [特点与区别](#4.9)
  - [TCP 三次握手与四次挥手](#4.3)
- [七层模型](#4.13)
