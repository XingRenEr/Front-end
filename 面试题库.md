...
- 代码执行到函数时，函数的上下文被推到**上下文栈**/**函数调用栈**。
  ...
  > 外层包围函数的**包含上下文**首先被推到上下文栈，当代码执行到嵌套的内层函数时，内层函数的上下文被推到上下文栈。
  >
  > **上下文栈的创建**：代码执行到函数时创建
...
- [x] [为什么使用token？session与token的区别](https://blog.csdn.net/mydistance/article/details/84545768)
  ...
  <img src="面试题库.assets/20181126151900484.png" alt="img" style="zoom: 67%;" />
...
- [x] [单点登录中 Token 认证的优势何在？](https://www.freebuf.com/articles/security-management/248647.html)
...
<img src="JavaScript/基础/images/offsetWidth.png" alt="picture" style="zoom:50%;" /><img src="JavaScript/基础/images/clientWidth.png" alt="picture" style="zoom:50%;" /> <img src="JavaScript/基础/images/scrollWidth.png" alt="picture" style="zoom:50%;" />  
...
* [x] [深入 CommonJs 与 ES6 Module](https://segmentfault.com/a/1190000017878394)

  > UMD

**概要**

前端模块化就是复杂的文件编程一个一个独立的模块（比如 js 文件），这有利于重用（复用性）和维护（版本迭代）。分成独立的模块会引来模块之间相互依赖和命名冲突的问题，所以有了 CommonJS 规范，AMD，CMD 规范等等，以及用于 js 打包（编译等处理）的工具 webpack
...
> 保证不同模块可以使用相同的变量名。
>
> 原理：闭包（函数式编程）。Node利用JavaScript的函数式编程的特性，实现了模块的隔离。
...
- 返回一个**新的** `Promise` 实例
  ...
  - 若没有 `onResolved` / `onRejected` 处理程序：`Promise.resolve()`包装上一个 `Promise` 实例状态转变后的值，即上一个 `Promise` 实例原样向后传
...
- `requestAnimationFrame` 会把每一帧中的所有DOM操作集中起来，在一次重绘中就完成（**一次性**）
...
将多个 `Promise` 对象组合成一个 `Promise` 对象的静态方法

**`Promise.all()`**

接收一个可迭代对象，返回一个新 `Promise` 对象
...
let p2 = Promise.all([3, 4]); // 可迭代对象中的元素会通过 Promise.resolve() 转换为 Promise 对象
let p3 = Promise.all([]); // 空的可迭代对象等价于 Promise.resolve()
let p4 = Promise.all(); // 无效的语法
```

全部 `resolve` 之后再 `resolve`。合成 `Promise` 对象的 `resolved` 值就是所有包含期约 `resolved` 值的数组（按照迭代器顺序）
...
如果至少有一个包含的 `Promise` 对象待定，则合成的 `Promise` 对象也会待定；如果有一个包含的 `Promise` 对象拒绝，则合成的 `Promise` 对象也会拒绝，将第一个拒绝的 `Promise` 对象的理由作为合成 `Promise` 对象的拒绝理由，其他拒绝操作被静默处理
...
接收一个可迭代对象，返回一个新 `Promise` 对象，是一组集合中最先解决或拒绝的 `Promise` 对象的镜像。

与 `Promise.all()` 类似，合成的 `Promise` 对象会静默处理所有包含的 `Promise` 对象的拒绝操作

### <a id="1.58"></a>1.58 箭头函数与普通函数的区别  

### <a id="1.59"></a>1.59 0.1+0.2不等于0.3

![image-20210926213130074](面试题库.assets/image-20210926213130074.png)

一个浮点数转换成带有尾数的二进制存储，若位数部分超过52位，会被截取，因此丢失精度。

![image-20210926213638206](面试题库.assets/image-20210926213638206.png)

![image-20210926213744188](面试题库.assets/image-20210926213744188.png)
...
小数位只能取前十七位，四舍五入后恰好为0.2

### <a id="1.60"></a>1.60 for 与 forEach() 的区别

在forEach中，不能使用 continue 和 break 跳出循环。

**跳出本次循环**

使用 return，效果相当于 for 的 continue
...
不能在内层使用 catch 捕获异常，这样只结束内层 forEach；需要在最外层捕获异常，结束嵌套循环
...
} catch (e) { //在最外层捕获异常，结束嵌套循环

}
```

### <a id="1.61"></a>1.61 如何避免表单重复提交

<img src="面试题库.assets/image-20211014104538192.png" alt="image-20211014104538192" style="zoom: 67%;" />
...
- [x] [对Flex布局的理解；动画演示](https://segmentfault.com/a/1190000022455712)

> 注：
>
> - 根据官方文档，`flex: 1;`代表 `flex: 1 1 0;`。下面说明与`flex: auto;`区别：
>
>   - 一个容器中有三个子元素，全部设置`flex: 1;`，则等分；若全部设置`flex: auto;`，则按比例分配
>   - 一个容器中有两个子元素，其中一个元素定宽。另一个子元素设置为`flex: 1;` / `flex: auto;`，效果相同
>
> - 若要 `align-items: stretch;` 生效，必须设置子元素`height: auto;`，否则 height 属性将会覆盖该 stretch
>
> - 对于 `align-items: baseline` ，若子元素内无内容，就会按照每个矩形的底部对齐
>
>   <img src="面试题库.assets/image-20210702105903696.png" alt="image-20210702105903696" style="zoom:50%;" /><img src="面试题库.assets/image-20210702105913489.png" alt="image-20210702105913489" style="zoom:50%;" />

### <a id="2.4"></a>2.4 BFC  
- [x] [**BFC**](https://www.jianshu.com/p/66632298e355)  
...
2. 哪些元素会生成 BFC / 如何**创建 BFC**  
   ...
   - display为inline-block, table-cell, table-caption, flex, inline-flex, flow-root...  
     ...
     > 相比float浮动，position绝对定位，或者overflow隐藏，inline-block等特性产生的块级格式上下文，`display:flow-root`不会给元素带来额外的副作用，例如`overflow:hidden`虽然可以去除浮动的干扰，但是，有可能会让子元素无法定位在容器外部。
     >
     > 例如：
     >
     > ![浮动高度塌陷示意](面试题库.assets/2020-05-12_224426.png)![浮动被清除了示意](面试题库.assets/2020-05-12_224628.png)
...
- 已知子元素大小：定位+根据子元素大小，计算 `margin` (不太好)  
  ```css
  .box {
    position: relative;
  }
...
- 已知子元素及父元素大小：根据子元素大小和容器大小，计算 `margin` (不好)  

- 设置子元素 `display: inline-block;`，设置容器 `display: table-cell;`  

  > 对`table-cell`元素设置百分比宽高值时无效
  >
  > - [x] [`display: table-cell;`实现水平垂直居中](https://www.cnblogs.com/yangshifu/p/6954364.html)
  ...
  p {
      display: inline-block; /* 这样父元素的text-align: center;才会生效 */
  }
  ```
  
  对`<p></p>`标签中的文字水平垂直居中，不需要设置 `display: inline-block;`
  ...
  ![image-20210701154804745](面试题库.assets/image-20210701154804745.png)![image-20210701154925208](面试题库.assets/image-20210701154925208.png)

- 已知父元素高度：设置子元素 `display: inline-block;`
  ...
  > `vertical-align: middle;`能应用于两个情景：
  >
  > - 应用于`inline/inline-block`元素，该元素**近似**垂直居中
  > - 应用于`table-cell`元素（其实就是`td`元素），若子元素为`inline-block`，则**近似**垂直居中；否则垂直居中
  >
  > - [x] [`vertical-align: middle;`近似垂直居中](https://alexzhong22c.github.io/2019/02/05/vertical-align-middle/)
...
- 已知子元素大小：`fixed` 定位+根据子元素大小，计算 `margin` (不太好)  
...
- `margin`用视口单位 `vh`+`transform` (独有方法)(还不错)  
...
| 可以设置 `width`、`height`、`margin` 和 `padding` | 只可以设置水平方向的 `margin` 和 `padding` |
...
|属性值|名称|文档流|覆盖其他元素|说明|
| - | - | - | - | - |
|`relative`|相对定位|普通流，占据空间|移动导致覆盖|`top, bottom, left, right`相对于原位置进行移动|
|`absolute`|绝对定位|脱离文档流，不占据空间|覆盖|`top, bottom, left, right`相对于最近已定位父元素 (即非 `static`) / `<html>`定位<br>`z-index`生效|
|`fixed`|固定定位|脱离文档流，不占据空间|覆盖|`top, bottom, left, right`相对于浏览器窗口定位<br>`z-index`生效|
...
- `sticky`用法  
  ...
  ![](面试题库.assets/15.jpg)  ![](面试题库.assets/16.jpg)    

**`z-index`**

设置元素的堆叠顺序  

可以被设定为关键词 `auto` 或 `<integer>`，使用负值降低优先级。
...
- [x] [参考链接](https://www.cnblogs.com/coco1s/p/7358830.html)
...
1. ID 选择器 > 类选择器 = 属性选择器 = 伪类选择器 > 标签选择器 = 伪元素选择器 

2. 当一个标签同时被多个选择符选中，确定优先级：

   计算 ID 选择器的个数（a），类选择器、属性选择器以及伪类选择器的个数之和（b），标签选择器和伪元素选择器的个数之和（c）。

   按 a、b、c 的顺序依次比较大小，大的则优先级高，相等则比较下一个。

   若两个的选择符中 a、b、c 都相等，则按照"就近原则"来判断。

   > 错误说法：
   >
   > ID 选择器权值为 100，类选择器权值为 10，标签选择器权值为 1，当一个选择器由多个 ID 选择器、类选择器或标签选择器组成时，则将所有权值相加，然后再比较权值。
...
```
- float (计算多)  
  ![image-20210630212546528](面试题库.assets/image-20210630212546528.png)
  ...
  .right {
    width: calc(100% - 100px - 100px - 10px);
  }
  ```
...
| 选择器          | 含义                                           | CSS 版本 | 应用                                             |
| :-------------- | :--------------------------------------------- | -------- | ------------------------------------------------ |
| E::after        | 创建一个伪元素，作为已选中元素的最后一个子元素 | CSS 2.1  | 清除浮动                                         |
| E::before       | 在E元素之前插入生成的内容                      | CSS 2.1  | `iconfont`的CSS文件为元素设置内容（Unicode编码） |
...
常用的：`none`,`block`,`inline`,`inline-block`,`flex`,`grid`,`table`,`table-cell`,`flow-root`
...
定义元素（外部元素）的内部元素的显示类型：

`flex`,`grid`,`table`,`flow-root`
...
CSS 2 对`display`属性使用单关键字语法（在 CSS 3 中有对应的多关键字，支持性差）：
...
- [x] [虚拟 DOM](https://juejin.cn/post/6844903870229905422)
...
2. 抽象了原本的渲染过程，实现了跨平台的能力（最大的优势）

   而不仅仅局限于浏览器的 DOM，可以是安卓和 IOS 的原生组件，可以是近期很火热的小程序，也可以是各种GUI

### <a id="3.2"></a>3.2 Vue 生命周期

<img src="面试题库.assets/99.jpg" alt="99" style="zoom: 50%;" />

- [x] [Vue面试题：生命周期函数](https://juejin.cn/post/6844903945530245128)

##### 什么是Vue生命周期

Vue 实例从创建到销毁的过程，就是生命周期。

也就是从开始创建、初始化数据、编译模板、挂载DOM-渲染、更新-渲染、卸载等一系列的过程

##### Vue生命周期的作用

Vue 所有的功能的实现都是围绕其生命周期进行的，在生命周期的不同阶段调用对应的钩子函数，可以实现组件数据管理和DOM渲染两大重要功能。 生命周期中有多个事件钩子，在控制整个vue实例的过程时更容易形成好的逻辑。

##### 流程

1. 创建之前 `beforeCreate`

   调用`new Vue()`初始化实例后，执行`beforeCreate`，此时数据未挂载，只是一个空壳，无法访问到数据和 DOM，不做操作

2. 创建完成 `created`

   创建实例、完成数据`(data、props、computed)`的初始化导入依赖项后，执行`created`，此时可访问、修改数据

   使用场景：在这里更改数据不会触发`updated`，是渲染前倒数第二次更改数据的机会。一般在这里做初始数据的**获取**，

   ​					初始化完成时的**事件**写在这里，

   ​					**异步请求**也适宜在这里调用（请求不宜过多，避免白屏时间太长）

3. 挂载之前 `beforeMount`

   编译模板，创建虚拟 DOM 准备渲染，然后执行 `beforeMount`

   使用场景：（在这里更改数据不会触发`updated`，是渲染前最后一次更改数据的机会。但一般不在这里做初始数据的获取）

   ​					这个阶段是过渡性的，一般一个项目只能用到一两次

4. 挂载完成 `mounted`

   完成挂载 DOM 和渲染、完成双向绑定，然后执行`mounted`

   使用场景：可以在这里**发送 Ajax**，以及执行依赖DOM的操作：如**操作 DOM**、绑定事件
...
### <a id="3.3"></a>3.3 移动APP(原生APP/APP) 与 web APP  
...
> 无状态：服务端不保存任何客户端请求者信息，客户端的每次请求必须具备自述信息，通过这些信息识别客户端身份。
>
> **优点**：客户端请求不依赖服务端的信息，即任何多次请求不需要必须访问到同一台服务器。这样为服务器横向扩展提供了条件。

##### HTTP报文结构

<img src="面试题库.assets/image-20211029151758415.png" alt="image-20211029151758415" style="zoom:70%;" />

##### [HTTP1.0第六版(目前使用)/衍生的HTTP1.1/HTTP-NG/HTTP2.0/HTTPS的特点](#4.12) 

##### 用于客户端发送[请求](#4.4)到服务端；服务端发送[响应](#4.16)到客户端  

##### 请求方法  

##### [状态码](#4.6)  

##### [常见头部(通用首部/请求首部/响应首部/实体首部)](#4.7)  
...
2. 对称加密
   ...
   如果每一个客户都有一个独立的key就好了，不现实

3. 非对称加密

   服务器有私钥，并把公钥以明文方式传输给浏览器
...
4. 非对称 + 对称
   ...
   先使用非对称加密，临时产生key（每一个客户都有一个独立的key）；

   再使用对称加密，利用key进行数据的交互 

   ![image-20210713173756127](面试题库.assets/image-20210713173756127.png)

   黑客攻击——中间人问题：

   根本原因是浏览器无法确认收到的公钥是不是网站自己的

   ![image-20210713185339737](面试题库.assets/image-20210713185339737.png)

5. 非对称 + 对称 + CA 机构数字证书

   解决——权威的CA机构认证通过的公钥才是好公钥

   csk对pk进行加密，生成license（证书）

   <img src="面试题库.assets/image-20210713190129107.png" alt="image-20210713190129107" style="zoom:80%;" />

   证书由服务器传递给客户端的过程中，防止被篡改：**数字签名**

   - [x] [数字签名是什么？](https://www.ruanyifeng.com/blog/2011/08/what_is_a_digital_signature.html)
   - [x] [彻底搞懂HTTPS的加密原理（数字签名）](https://zhuanlan.zhihu.com/p/43789231)

   <img src="面试题库.assets/image-20210915170455682.png" alt="image-20210915170455682" style="zoom: 80%;" />

6. 非对称 + 对称 + hash + CA 机构数字证书

   **生成对称加密key的详细过程**

   - [x] [SSL/TLS协议运行机制的概述——阮一峰](https://www.ruanyifeng.com/blog/2014/02/ssl_tls.html)

   <img src="面试题库.assets/image-20210713192202659.png" alt="image-20210713192202659" style="zoom:50%;" />

   **为什么一定要用三个随机数，来生成"会话密钥"：**

   > "由于SSL协议中证书是静态的，因此十分有必要引入一种随机因素来保证协商出来的密钥的随机性。不管是客户端还是服务器，都需要随机数，这样生成的密钥才不会每次都一样。
   >
   > 对于RSA密钥交换算法来说，pre-master key本身就是一个随机数（第三个随机数），再加上hello消息中的随机（前两个随机数），三个随机数通过一个密钥导出器最终导出一个对称密钥。
   >
   > 一个伪随机可能完全不随机，可是是三个伪随机就十分接近随机了，每增加一个自由度，随机性增加的可不是一。"

   **HTTPS SSL 四次握手（有图更直观）**

   - [x] [HTTP与HTTPS的区别, 以及SSL四次握手过程](https://blog.csdn.net/c_kite/article/details/79826939)

   ![这里写图片描述](面试题库.assets/20180405155451252)

**每次进行HTTPS请求时都必须在SSL/TLS层进行握手传输密钥吗？**

session ID：服务器会把该密钥存到相应的session ID下，，之后浏览器每次请求都会携带session ID，服务器会根据session ID找到相应的密钥并进行解密加密操作，这样就不必要每次重新制作、传输密钥了

[**HTTP 与 HTTPS 的区别**](#4.10)

### <a id="4.3"></a>4.3 TCP 三次握手与四次挥手  
**一句话概括三次握手/四次挥手**

客户端和服务端各自都需要确认自己可收发，因此需要三次握手。  

> 为什么不是两次：防止已失效的连接请求又传送到服务器端，因而产生错误（《计算机网络》）

客户端和服务端各自都需要告诉对方不再发送数据（受到上层控制）并收到对方回应，因此需要四次挥手。  

<img src="面试题库.assets/5.png" alt="简化三次握手" style="zoom: 67%;" />  <img src="面试题库.assets/image-20210604162446893.png" alt="image-20210604162446893" style="zoom:67%;" />
...
|OPTIONS|1. 检测服务器支持哪些 HTTP 方法（通过 `Allow` 响应头）。<br>2. 在 CORS 中，用来发起[预检请求](#1.17)，检测实际请求是否可以被服务器所接受；服务器将所有允许的请求方法告知客户端（通过 `Access-Control-Allow-Methods` 响应头）。								|
|TRACE	|回显服务器收到的请求，主要用于测试或诊断。							|
|PATCH	|是对 PUT 方法的补充，用来对已知资源进行局部更新。				|

- [x] [GET 和 POST 的区别](#4.11)
...
- 提升性能  
  ...
  - **二进制分帧**：http1.X 的解析是基于文本（ASCII码）的，http2.0 将信息分割为更小的消息和帧，并采用二进制格式编码  

    - 二进制分帧便于多路复用，防止阻塞；即使是无序传输，接收的数据也可以在二进制帧层重组（利用序号）。

      <img src="面试题库.assets/image-20210713164435636.png" alt="image-20210713164435636" style="zoom:50%;" />

    - 报文帧分为 `header frame` 和 `data frame`。`header` 和 `data` 帧可以分为多个片段帧，例如 `data` 帧即是可以 `data = data_1 + data_2 + ... + data_n`。

      除了报文帧，还有SETTINGS、PING、GOWAY、WINDOW_UPDATE等控制帧。便于高级功能的扩展。
...
| 请求首部 | 有关要获取的资源或客户端本身信息 | 条件请求：[`Accept`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Accept)、`Accept-*`、**`If-*`**，<br>描述请求本身(以确保服务端返回正确响应)：[`Cookie`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cookie)、[`User-Agent`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/User-Agent)、[`Referer`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Referer) |
...
|`If-Match`							|	|	|
|`Origin`								|发起一个针对跨域资源共享的请求（该请求要求服务器在响应中加入一个`Access-Control-Allow-Origin`的消息头，表示访问控制所允许的来源）。|Origin: http://www.itbilu.com			|
...
- `Content-Type`
  ...
  - `application/x-www-form-urlencoded`
    ...
    HTTP 请求体用 key1=val1&key2=val2 的方式进行组织。

    一般用于表单提交

  - `multipart/form-data`

    多部分多媒体类型。在 HTTP 请求报文中，多个参数将由------boundary分割，形成多个参数块。每个块中由附加信息Content-Disposition(form-data或者file)、空行和参数内容构成。若参数是文件，附加信息还会包含Content-Type。
...
<img src="浏览器/img/cache-5.png" alt="浏览器缓存" style="zoom: 25%;" />  

#### 相关响应首部及请求首部

<img src="面试题库.assets/v2-49861b3ea887182cdaee0b9732841d31_720w.jpg" alt="img" style="zoom:50%;" />

**强制缓存**

判断是否缓存的依据来自于是否超出某个时间或者某个时间段，不关心服务器端文件是否已经更新，这可能会导致加载文件不是服务器端最新的内容。

表示方式：返回 `200` 状态码，并且 `Size` 显示为 `from disk cache` 或者 `from memory cache`。

- `Cache-Control`（可为响应首部、请求首部）

  在请求首部和响应首部中的区别：  

  只有服务端才能开启缓存，默认不缓存  

  客户端的请求头中只有设置为以下值才会有效（即客户端不想走强缓存时生效） 

  - `no-store`
  - `no-cache`（eg.浏览器按ctrl+F5强制刷新）
  - `max-age=0` 
  ...
  |`no-store`|不使用任何缓存(强制缓存/协商缓存)。|
  |`max-age=<seconds>`|设置缓存存储的最大周期，超过这个时间缓存被认为过期(单位秒)。|
  |`s-maxage=<seconds>`|优先级高于 max-age，仅适用于代理服务器，对浏览器不起作用|


- `Expires`（响应首部）

  GMT 格式的时间，来明确地宣告资源的有效期。

**协商缓存**

- `Etag`（响应首部）与`If-None-Match`（请求首部）

  二者的值都是服务器为每份资源分配的唯一标识字符串。

  - 资源更新时，服务器端的`ETag`值也随之更新；
  - 浏览器首次请求资源，服务器在响应头中加入`ETag`字段。
  - 浏览器再次请求资源，在请求头中加入`If-None-Match`字段，取值为上次响应头的`ETag`的值；
  - 服务器比对`ETag`与`If-None-Match`的值是否一致，
    - 如果不一致，返回更新后的资源；
    - 如果一致，表明资源未更新，返回状态码为`304`的响应，可继续使用本地缓存。
    - 此时响应头会加上`ETag`字段，即使它没有变化。

- `Last-Modified`（响应首部）与`If-Modified-Since`（请求首部）

  二者的值都是GMT格式的时间字符串。

  - 浏览器首次请求资源，服务器在响应头中加入`Last-Modified`字段，表示该资源最后一次的修改时间；
  - 浏览器再次请求资源，在请求头中加入`If-Modified-Since`字段，取值为上次响应头的`Last-Modified`的值；
  - 服务器比对`Last-Modified`与`If-Modified-Since`的值是否一致，
    - 如果不一致，返回更新后的资源；
    - 如果一致，表明资源未更新，返回状态码为`304`的响应，可继续使用本地缓存。
    - 与`ETag`不同的是：此时响应头不会再添加`Last-Modified`字段。

### <a id="4.9"></a>4.9 TCP、UDP 的区别  

- [x] [一文搞定 UDP 和 TCP 高频面试题！](https://zhuanlan.zhihu.com/p/108822858)

|TCP||UDP||
| - | - | - | - |
|面向连接||无连接|即发送数据前不需要先建立链接|
|提供可靠交付|无差错，不丢失，不重复，按序到达|尽最大努力交付|即不保证可靠交付|
|面向字节流|把应用层传下来的报文看成字节流，把字节流组织成大小不等的数据块|面向报文|对于应用程序传下来的报文不合并也不拆分，只是添加 UDP 首部|
|有流量控制，拥塞控制||没有拥塞控制|会出现丢包，应用：实时传输如 IP 电话、视频会议等|
|一对一，提供全双工通信||支持一对一、一对多、多对一和多对多||
|首部20-60字节||首部8字节||
...
|      | 示意图                                                       | 字节数                                            | 内容                           |
| ---- | ------------------------------------------------------------ | ------------------------------------------------- | ------------------------------ |
| UDP  | ![preview](面试题库.assets/v2-c01a6511abf81f867fd0e531bd51af2c_r.jpg) | 8字节（12字节的伪首部是为了计算检验和临时添加的） | 源端口、目的端口、长度、检验和 |
| TCP  | ![preview](面试题库.assets/v2-3bd45a13afe868cae6225d75b85e9c36_r.jpg) | 20（固定首部）-60字节                             | 见下                           |
...
<img src="面试题库.assets/image-20210713161828869.png" alt="image-20210713161828869" style="zoom: 80%;" />
...
|端口|80|443|
|优缺点||**安全性**：①可认证用户和服务器，确保数据发送到正确的客户机和服务器 ②可进行加密传输、身份认证，可防止数据在传输过程中不被窃取、改变，确保数据的完整性和真实性<br>**消耗资源**：③握手阶段比较费时，页面加载时间延长50%，耗电增加10%~20% ④消耗缓存，增加数据开销 ⑤功能越强大的 SSL 证书费用越高 ⑥SSL 证书需要绑定 IP，不能在同一个 ip 上绑定多个域名，ipv4 资源支持不了这种消耗???|
...
* 大小限制：`GET` 请求长度在浏览器中有限制，而 `POST` 并没有。
  ...
  > 这里的长度限制，指的是整个URL长度，而不是 `GET` 请求传参长度；HTTP协议从未规定GET/POST的请求长度限制是多少。对 `GET` 请求参数的限制来源于浏览器或者 `Web` 服务器。传统IE中URL的最大可用长度为2048字符，其他浏览器对URL长度限制实现上有所不同  
* 缓存：`GET` 请求会被浏览器主动保留下来，而 `POST` 默认不会。

  > get 请求类似于查找的过程，用户获取数据，可以不用每次都与数据库连接，所以缓存。
  > post 做的一般是修改和删除的工作，必须与数据库交互，所以不缓存。

### <a id="4.12"></a>4.12 HTTP1.0、1.1、2.0 的区别、新增的字段及作用  

##### `HTTP1.1`

* 长连接。`HTTP/1.1` 默认开启 `Connection: keep-alive`，让一个 `TCP` 连接能重复发送/接收多次 `HTTP` 请求，缩短多个请求的总响应时间。
...
1. 纯文本 VS 二进制
   ...
   <img src="面试题库.assets/image-20210713163159754.png" alt="image-20210713163159754" style="zoom:67%;" />

   HTTP1.0规定头信息必须是ASCII码（即纯文本），后面的数据可以是任何格式。

2. 长连接复用 VS 多路复用

   <img src="面试题库.assets/520a0ed7a9bc05a1e51d1af5065f0c961603447114760" alt="［转］HTTP1.0、HTTP1.1 和 HTTP2.0 的区别5" style="zoom: 67%;" />

##### [`HTTPS`](#4.2)

### <a id="4.13"></a>4.13 OSI七层模型、TCP/IP四层模型、五层模型  

![图](.\%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C\img\network-architecture-2.png?lastModify=1623120930)

![图](.\%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C\img\network-architecture.png?lastModify=1623120964)

### <a id="4.14"></a>4.14 DNS

![img](面试题库.assets/464291-20170703113844956-354755333.jpg)
...
### <a id="4.15"></a>4.15 HTTP 和 TCP 区别

HTTP 是应用层协议，定义的是如何包装数据的规范；而 TCP 是传输层协议，定义的是连接方式和数据传输的规范。

HTTP 协议中的数据是利用 TCP 协议传输的，所以支持 HTTP 的也一定支持 TCP。

### <a id="4.16"></a>4.16 HTTP 响应
...
### <a id="5.1"></a>5.1 异步加载 js 的方法；async 和 defer 区别  
...
脚本：蓝色线代表加载，红色线代表执行；HTML 解析：绿色线。

|              | defer                                         | async                                         |
| ------------ | --------------------------------------------- | --------------------------------------------- |
| 产生         | 适用于所有主流浏览器                          | H5 新属性                                     |
| 用法         | `<script src='www.baidu.com' defer></script>` | `<script src='www.baidu.com' async></script>` |
| 脚本执行时间 | HTML 解析完成后                               | 该 JS 脚本加载完                              |
| 脚本执行顺序 | 按照 JS 脚本声明的顺序                        | 乱序                                          |
| 适用范围     | 所有脚本                                      | 外部脚本（即script有src属性）                 |
| 应用         | 依赖于 DOM 元素或其他 JS                      | 和其他脚本和 DOM 元素依赖不强                 |

### <a id="5.2"></a>5.2 CDN 的使用

静态资源的请求可以采用 `CDN`，减少服务器压力、防止不必要携带 `Cookie` 的场景等。

### <a id="5.3"></a>5.3 CSS 优化
...
- [本地存储](#1.26)
- [（CDN 的使用）](#5.2)
- 加载 CSS：尽可能在 `head` 位置加载 CSS，减少 HTML 加载完毕需要等待 CSS 加载的问题。
- 加载 JS：内部 `script` 脚本通常放 `body` 后面，外部脚本可利用 [async 和 defer](#5.1) 属性
- 预加载
- 懒加载：针对长列表中的图片

### <a id="5.5"></a>5.5 服务端渲染

更好的 SEO（供搜索引擎使用）

解决首屏加载速度过慢的问题

### <a id="5.6"></a>5.6 图片预加载

loading...的过程

**原理**

- `new Image()` 动态创建 img 标签
- 设置图片 src 属性
- 使用 onload 事件设置预加载完成的回调函数：将图片插入页面中

![image-20210917165632719](面试题库.assets/image-20210917165632719.png)![image-20210917170321753](面试题库.assets/image-20210917170321753.png)

### <a id="5.7"></a>5.7 懒加载：长列表含图片

**是什么**：包含图片的长列表，优先加载、渲染可视区域中的图片
...
- 方法一
  ...
  JS：`可视区域高度 - 元素顶部距离可视区域顶部的高度 > 0`，表示元素露出，设置 img 标签的 src 属性。监听 scroll 事件。

  > 可视区域高度：`window.innerHeight || document.documentElement.clientHeight`
  >
  > 元素顶部距离可视区域顶部的高度：`getboundingclientrect().top`
...
| 图片类型        | 关键字                               | 特点                                                         | 使用场景                                                     | 注                                                           |
| --------------- | ------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| JPEG 与 JPG     | 有损压缩、体积小、加载快、不支持透明 | **优点**：压缩一定程度能保持品质、体积小、请求速度快<br/>**缺点**：处理矢量图形、Logo 等线条感较强，颜色对比强烈的图形，人为压缩会导致图片模糊明显。不支持透明度处理。 | 大的背景图、轮播图或者 Banner 图                             |                                                              |
| PNG-8 与 PNG-24 | 无损压缩、质量高、体积大、支持透明   | **优点**：PNG-8 支持 256 种颜色，PNG-24 支持 1600 种颜色。更强的色彩表现力，对线条的处理更加细腻，对透明度有良好的支持。<br/>**缺点**：体积较大 | Logo、颜色简单且对比强烈的图片和背景                         |                                                              |
| GIF             | 动态图、体积小支持透明               | **优点**：可以压缩体积非常小。可插入多帧实现动画效果。支持透明色浮现于背景之上。<br/>**缺点**：最多只能处理 256 中颜色，不适用于真彩图像。 | 小动画。                                                     |                                                              |
| SVG             | 文本文件、体积小、不失真、兼容性好   | **优点**：文本体积更小，可压缩性更强。图片可以无限放大不失真。文本文件可以直接在 HTML 中写入，灵活性高。<br/>**缺点**：渲染成本高、学习成本（可编程） | 变成代码嵌入 HTML 中，也可以换成 `.svg` 后缀的文件进行引用。 |                                                              |
| Base64          | 文本文件、依赖编码、小图标解决方案   | **优点**：作为雪碧图的补充而存在，减少加载页面图片时对服务器的请求次数。（`img src` 会发起资源请求，但是 Base64 得到的是字符串，嵌入 HTML 中）<br/>**缺点**：大图使用 Base64 会增大体积，影响性能 | 小 Logo（不超过 2kb）、更新频率低的图片。                    | **编码工具**：Webpack 的 `url-loader` 可以根据文件大小来判断是否编码成 Base64。 |
| 雪碧图          |                                      | **优点**：是一种优化手段，因为单张图片所需的 HTTP 请求更少，对内存和带宽更加友好。<br/>**缺点**： | 小图标                                                       |                                                              |
| WebP            | 年轻的全能型选手                     | **优点**：支持有损压缩和无损压缩、支持透明、可以跟 GIF 一样显示动态图<br/>**缺点**：兼容性差 | 暂无大型应用场景                                             |                                                              |
...
**知识点**  
- [篡改攻击和重放攻击](#7.1)
- [XSS (Cross Site Scripting, 跨站脚本攻击)](#7.3)
- [CSRF (Cross Site Request Forgery, 跨站请求伪造)](#7.2)
- [iframe 安全]
- [SQL 注入]
- [ACK 攻击]

### <a id="7.1"></a>7.1 篡改攻击和重放攻击

- [x] [API接口防止参数篡改和重放攻击](https://juejin.cn/post/6890798533473992717#heading-2)
...
- [x] [保护ASP.NET 应用免受 CSRF 攻击](https://www.cnblogs.com/shanyou/p/5038794.html)

  防御手段

- [x] [前端安全系列（二）：如何防止CSRF攻击？](https://tech.meituan.com/2018/10/11/fe-security-csrf.html)

  典型 CSRF 例子

### <a id="7.3"></a>7.3 XSS

- [x] [XSS(跨站脚本攻击)详解](https://www.cnblogs.com/csnd/p/11807592.html)
  ...
  防御：客户端对用户的输入(和URL参数)进行过滤，对输出进行html编码，服务端设置会话Cookie的HTTP Only属性
...
- `DNS` 解析
  ...
  - 查询 `www.baidu.com`
  - 访问客户端 DNS 缓存：**浏览器 DNS 缓存** -> **系统缓存（host）** -> **路由器缓存**
...
- 浏览器解析渲染页面  
  ...
  - [x] [浏览器渲染页面的流程](https://www.bilibili.com/video/BV18f4y1H7Zu?from=search&seid=7079518874657661986&spm_id_from=333.337.0.0)

  <img src="面试题库.assets/image-20210917145839717.png" alt="image-20210917145839717" style="zoom:50%;" />

  若 `JS` 对 `DOM` 或 `CSSOM` 进行了修改，后面三步要重新执行。

  - 解析 `HTML`，构建 **`DOM`** 【遇到 `CSS` 加载 `CSS` ；遇到 `javascript` 加载 `javascript` ；遇到 `<img>` 标签下载图片资源，这些不影响 `HTML` 的解析】

    > `JS` 脚本默认同步加载、执行，会阻塞 `HTML` 的解析：
    >
    > <img src="面试题库.assets/image-20210917144423521.png" alt="image-20210917144423521" style="zoom:50%;" />
    >
    > [`async` 与 `defer`](#5.1)：如果为 `JS` 脚本加上 `async` 属性，会异步加载，立即执行；`defer` 属性，会异步加载、待 HTML 解析完成后执行。
    
  - 解析 `CSS`，构建 **`CSSOM`** 【遇到背景图片链接不加载】
  
    > `CSS` 包括外部样式，内部样式，内联样式
  - 将 `DOM` 和 `CSSOM` 结合，构建**渲染树**【加载背景图片】
  
    > 这里的主要目的是排除非视觉节点，比如 `script`、`meta` 标签、 `link` 标签、`display: none` 的节点  
  
  - **布局**：遍历计算元素位置、尺寸
  - **绘制**：遍历布局渲染树【开始渲染图片】  

还需要关注的就是**浏览器性能优化**、**[浏览器缓存](#4.8)**、**[跨域](#1.15)**、[**回流和重绘**](#11.12)  
...
- 多媒体元素  
  ...
  `<audio>`音频，`<video>`视频，`<source>`为 `video` 和 `audio` 指定 **媒体源**、`<track>` 为 `video` 和 `audio` 指定 **文本轨道**（字幕）
...
- 定时器 [`requestAnimationFrame`](#1.45)
...
- [Web Worker](#1.27)(多线程编程)

- [WebSocket](#1.19)(双向通信协议，可用于跨域)
...
- CSS：用 `visibility:hidden;` 替换 `display:none;`

- CSS：避免 `table` 布局。对于 `Render Tree` 的计算只需遍历一次，但 `table` 布局需要计算多次，要花 3 倍时间

- JS：避免频繁操作 `width`、`height`

- JS：将频繁操作改为一次性操作，创建一个文档片段 `document.createDocumentFragment()`，先将代码插入到文档片段中，最后一次性将内容插入DOM结构中

  > **虚拟DOM**就是为了解决浏览器性能问题而被设计出来的。如前，若一次操作中有10次更新DOM的动作，虚拟DOM不会立即操作DOM，而是将这10次更新的diff内容保存到本地一个JS对象中，最终将这个JS对象一次性attch到DOM树上，再进行后续操作，避免大量无谓的计算量。所以，用JS对象模拟DOM节点的好处是，页面的更新可以先全部反映在JS对象(虚拟DOM)上，操作内存中的JS对象的速度显然要更快，等更新完成后，再将最终的JS对象映射成真实的DOM，交由浏览器去绘制。
  >
  > **jQuery**时代提供的强大便利地操作DOM的API在前端工程里**用的越来越少**。
...
- [x] [`gulp` 与 `webpack` 区别](https://www.cnblogs.com/lovesong/p/6413546.html)

gulp：旨在规范前端开发流程，模块化不是它强调的东西。
