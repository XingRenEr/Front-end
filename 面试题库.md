...
|`absolute`|绝对定位|脱离普通流|否|覆盖|`top, bottom, left, right`相对于最近已定位父元素/`<html>`定位<br>`z-index`生效|
|`fixed`|固定定位|脱离普通流|否|覆盖|`top, bottom, left, right`相对于浏览器窗口定位<br>`z-index`生效|
