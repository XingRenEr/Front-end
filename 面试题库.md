...
2. `Promise.prototype.then()` / `Promise.prototype.catch()` / `Promise.prototype.finally()` 实例方法
   ...
   - 具体栗子：
     ...
     // Promise {<pending>}
     p1.then((res) => onResolved(res), () => onRejected());
     // ok
   ...
   - 返回一个**新的** `Promise` 实例
     ...
     若 `onResolved` 处理程序有返回值：`Promise.resolve()`包装 `onResolved` 处理程序的返回值

     

     若 `onResolved` 处理程序无返回值：`Promise.resolve()`包装 `undefined`

     若没有 `onResolved` 处理程序：`Promise.resolve()`包装传入 `resolve()` 的参数，即上一个 `Promise` 实例原样向后传
