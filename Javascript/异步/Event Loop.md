...
* `setTimeout` 的执行时间并不是确定的。它属于宏任务队列，只有当主线程上的任务执行完毕后，才会调用队列中的任务开始执行。
* 浏览器自身计时器的精度不准。
* 刷新频率受屏幕分辨率和屏幕尺寸影响，因此不同设备的刷新频率不同，而 `setTimeout` 只能固定一个时间间隔刷新。(刷新频率和 `setTimeout` 固定时间间隔可能对不上)
...
浏览器环境下，先执行 `microtask` ，再执行 `macrotask` 。
...
* [x] [Event Loop - 阮一峰](http://www.ruanyifeng.com/blog/2014/10/event-loop.html)
